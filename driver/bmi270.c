...
// 在 bmi270.c 中增加读取中断状态寄存器的函数
uint8_t read_int_status(struct bmi270 *sensor, uint8_t int_reg_addr) {
    return read_register(sensor, int_reg_addr);
}
...
#include "bmi270_config_file.h"  // 确保已包含配置数据

int load_config_file(struct bmi270 *sensor) {
    int rslt = 0;

    // 1. set INIT_CTRL = 0x00 (disable config loading)
    rslt |= write_register(sensor, 0x59, 0x00);

    // 2. set INIT_ADDR_0 = 0x00, INIT_ADDR_1 = 0x00 (start from beginning)
    rslt |= write_register(sensor, 0x5B, 0x00);
    rslt |= write_register(sensor, 0x5C, 0x00);

    // 3. Write full config file to INIT_DATA (0x5E)
    for (size_t i = 0; i < bmi270_config_file_len; ++i) {
        rslt |= write_register(sensor, 0x5E, bmi270_config_file[i]);
    }

    // 4. set INIT_CTRL = 0x01 (trigger config loading)
    rslt |= write_register(sensor, 0x59, 0x01);

    // 5. Wait for INTERNAL_STATUS (0x21) bit0 = 1
    int retry = 1000;
    while (retry--) {
        uint8_t status = read_register(sensor, 0x21);
        if (status & 0x01) {
            return 0;  // success
        }
    ...
    fprintf(stderr, " Timeout: BMI270 init status not ready!\n");
    return -1;
}
